...
#define JSON_SUPPORT_VECTOR	1	//支持libdsa中的vector类型
...
typedef struct key_info_s{
	...
	int arrayitem_size;				//成员每个元素大小
}json_kinfo_t;



/*成员在结构体中的偏移地址*/
#define NAME_OFFSET(type,name) ((unsigned long)(&(((type *)0)->name)))
#define NAME_SIZE(type,name) (sizeof((((type *)0)->name)))
#define ARRAY_CNT(arraycnt, type, name) (0<(arraycnt) ? NAME_SIZE(type,name)/(arraycnt) : 0)
 ...
 *
 */
#define MAKE_ARRAY_INFO(ctype, ktype, kname, subkey, arraycnt, arraytype) {sizeof(ctype), ktype, #kname, NAME_OFFSET(ctype,kname), NAME_SIZE(ctype,kname), subkey, arraycnt, arraytype, ARRAY_CNT(arraycnt,ctype,kname)}
#if (JSON_SUPPORT_VECTOR)
#define MAKE_VECTOR_INFO(ctype, ktype, kname, subkey, arraytype, arrayitem_size) {sizeof(ctype), ktype, #kname, NAME_OFFSET(ctype,kname), NAME_SIZE(ctype,kname), subkey, 0, arraytype, arrayitem_size}
#endif
#define MAKE_KEY_INFO(ctype, ktype, kname, subkey) MAKE_ARRAY_INFO(ctype, ktype, kname, subkey, 0, KEY_TYPE_NULL)
#define MAKE_END_INFO()	{0, KEY_TYPE_NULL, NULL, 0, 0, NULL, 0, KEY_TYPE_NULL, 0}
